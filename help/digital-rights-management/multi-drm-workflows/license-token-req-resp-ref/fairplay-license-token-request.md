...
| 쿼리 매개 변수 | 설명 | 필수? |
|--- |--- |--- |
| customerAuthenticator 고객 인증자를 쿼리 매개 변수 고객인증기 페어플레이 | 프로덕션 및 테스트 환경에 적합한 고객 API 키입니다. ExpressPlay 관리 대시보드 탭에서 확인할 수 있습니다. | 예 |
| errorFormat | html 또는 json 중 하나. html(기본값)인 경우 응답의 엔티티 본문에 모든 오류에 대한 HTML 표현이 제공됩니다. json이 지정된 경우 JSON 형식의 구조화된 응답이 반환됩니다. 자세한 내용은 [JSON 오류](https://www.expressplay.com/developer/restapi/#json-errors)를 참조하십시오. 응답의 MIME 유형은 success의 텍스트/uri-list, HTML 오류 포맷의 텍스트/html 또는 JSON 오류 포맷의 application/json입니다. | 아니요 |
...
| `kek` | 키 암호화 키(KEK). 키는 키 흐름 알고리즘을 사용하여 KEK를 사용하여 암호화됩니다(AES 키 랩, RFC3394). `kek`이(가) 제공되면 `kid` 또는 `ek` 매개 변수 중 하나를 제공해야 하지만 *은(는) 모두*&#x200B;은(는) 제공해야 합니다. | 아니요 |
| `kid` | 컨텐츠 암호화 키 또는 문자열 `'^somestring'`의 16바이트 16진수 문자열 표현입니다. 문자열 뒤에 `'^'`이 오는 길이는 64자보다 길 수 없습니다. | 아니요 |
...
| `fpExtension` | `extensionType` 및 `extensionPayload`을(를) 쉼표로 구분된 문자열로 둘러싸는 짧은 양식입니다. 예:[..] `&fpExtension=wudo,AAAAAA==&`[..] | 아니오 아무 번호나 사용할 수 있습니다 |
...
<table id="table_ar3_lsx_pv">  
 ...
 <tbody> 
  <tr> 
   ...
   <td> 이 토큰의 만료 시간입니다. 이 값은 'Z' 영역 지정자("Zulu time")에 있는 <a href="https://www.ietf.org/rfc/rfc3339.txt" format="html" scope="external"> RFC 3339 </a> 날짜/시간 형식의 문자열이거나 '+' 기호 앞에 오는 정수여야 합니다. RFC 3339 날짜/시간의 예는 <span class="codeph"> 2006-04-14T12:01:10Z </span>입니다. <p>값이 <a href="https://www.ietf.org/rfc/rfc3339.txt" format="html" scope="external"> RFC 3339 </a> 날짜/시간 형식의 문자열이면 토큰에 대한 절대 만료 날짜/시간을 나타냅니다. 값이 '+' 기호 앞에 오는 정수인 경우 발행부터 해당 토큰이 유효하다는 상대 시간(초)으로 해석됩니다. </p> 예를 들어 <span class="codeph"> +60 </span>은 1분을 지정합니다. 최대 및 기본(지정되지 않은 경우) 토큰 수명은 30일입니다. </td> 
   <td> 아니요 </td> 
  </tr> 
 </tbody> 
</table>
...
| **쿼리 매개 변수** | **설명** | **필수?** |
|---|---|---|
| `cookie` | 토큰에 들어 있고 토큰 상환 서버에서 로깅한 최대 32자 길이의 임의 문자열입니다. 이 ID를 사용하여 등록 서버와 서비스 공급자의 서버에 있는 로그 항목을 상호 연관시킬 수 있습니다. | 아니요 |
...
<table id="table_i2c_zsx_pv">  
 ...
 <tbody> 
  ...
  <tr> 
   <td> -2018년 </td> 
   <td> 인증 토큰이 잘못되었습니다.&lt;details&gt; <p>참고: 인증자가 잘못되거나 제작 인증자를 사용하여 <span class="filepath"> *.test.express.com </span>에서 테스트 API에 액세스할 때 또는 그 반대의 경우일 수 있습니다. </p> <p importance="high">참고: 테스트 SDK 및 고급 테스트 도구(ATT)는 <span class="filepath"> *.test.express.play.com </span>에서만 작동하지만 프로덕션 장치는 <span class="filepath"> *.service.express.com </span>을 사용해야 합니다. </p> </td> 
  </tr> 
  <tr> 
   <td> -2019년 </td> 
  ...
  <tr> 
   <td> -4021 </td> 
   <td> <span class="codeph"> 키드는 "^" 뒤에 64자여야  </span> 합니다. </td> 
  </tr> 
  <tr> 
   <td> -4022 </td> 
  ...
  <tr> 
   <td> -6006 </td> 
   <td> FairPlay 지원에 대해 인증되지 않은 서비스 </td> 
  </tr> 
  <tr> 
   <td> -6007 </td> 
  ...
  <tr> 
   <td> -6008 </td> 
   <td> FairPlay에서 장치 ID 바인딩이 지원되지 않습니다. </td> 
  </tr> 
  <tr> 
   <td> -6009 </td> 
