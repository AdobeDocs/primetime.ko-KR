...
| errorFormat | html 또는 json 중 하나. html(기본값)이 응답의 엔티티 본문에 오류의 HTML 표현을 제공하는 경우 json이 지정된 경우 JSON 형식의 구조화된 응답이 반환됩니다. 자세한 내용은 [JSON 오류](https://www.expressplay.com/developer/restapi/#json-errors)를 참조하십시오. 응답의 MIME 유형은 success의 텍스트/uri-list, HTML 오류 포맷의 텍스트/html 또는 JSON 오류 포맷의 application/json입니다. | 아니요 |
...
| `kek` | 키 암호화 키(KEK). 키는 키 래핑 알고리즘(AES 키 랩, RFC3394)을 사용하여 KEK로 암호화됩니다. `kek`이(가) 제공되면 `kid` 또는 `ek` 매개 변수 중 하나를 제공해야 하지만 *이(가) 모두*&#x200B;는 아닙니다. | 아니요 |
| `kid` | 컨텐츠 암호화 키 또는 문자열 `'^somestring'`의 16바이트 16진수 문자열 표현. 문자열 뒤에 `'^'`가 오는 길이는 64자보다 길 수 없습니다. | 아니요 |
...
| `fpExtension` | 쉼표로 구분된 문자열로 `extensionType` 및 `extensionPayload`을 둘러싸는 짧은 양식입니다. 예:[..] `&fpExtension=wudo,AAAAAA==&`[..] | 아니오, 아무 번호나 사용할 수 있습니다 |
...
<table id="table_ar3_lsx_pv">  
 ...
 <tbody> 
  <tr> 
   ...
   <td> 이 토큰의 만료 시간입니다. 이 값은 'Z' 영역 지정자("Zulu time")에 있는 <a href="https://www.ietf.org/rfc/rfc3339.txt" format="html" scope="external"> RFC 339 </a> 날짜/시간 형식의 문자열이거나 '+' 기호 앞에 있는 정수여야 합니다. RFC 3339 날짜/시간의 예는 <span class="codeph"> 2006-04-14T12:01:10Z </span>입니다. <p>값이 <a href="https://www.ietf.org/rfc/rfc3339.txt" format="html" scope="external"> RFC 339 </a> 날짜/시간 형식의 문자열인 경우, 토큰의 절대 만료 날짜/시간을 나타냅니다. 값이 '+' 기호 앞에 있는 정수인 경우 발행부터 토큰의 유효성이 상대 시간(초)으로 해석됩니다. </p> 예를 들어 <span class="codeph"> +60 </span>은 1분을 지정합니다. 최대 및 기본(지정되지 않은 경우) 토큰 수명은 30일입니다. </td> 
   <td> 아니요 </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_i2c_zsx_pv">  
 ...
 <tbody> 
  ...
  <tr> 
   <td> -2018 </td> 
   <td> 인증 토큰이 잘못되었습니다.&lt;details&gt; <p>참고: 인증자가 잘못되거나 제작 인증자를 사용하여 <span class="filepath"> *.test.express.com </span>에서 테스트 API에 액세스할 때 발생할 수 있습니다. </p> <p importance="high">참고: 테스트 SDK 및 고급 테스트 도구(ATT)는 <span class="filepath"> *.test.express.com </span>에서만 작동하지만 프로덕션 장치에서는 <span class="filepath"> *.service.express.com </span>을 사용해야 합니다. </p> </td> 
  </tr> 
  <tr> 
   <td> -2019년 </td> 
  ...
  <tr> 
   <td> -4021 </td> 
   <td> <span class="codeph"> 키드는 이전 64자여야  </span> 합니다. </td> 
  </tr> 
  <tr> 
   <td> -4022 </td> 
