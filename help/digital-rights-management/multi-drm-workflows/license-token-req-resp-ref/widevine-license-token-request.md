---
description: Widevine 라이선스 토큰 인터페이스는 프로덕션 및 테스트 서비스를 제공합니다.
seo-description: Widevine 라이선스 토큰 인터페이스는 프로덕션 및 테스트 서비스를 제공합니다.
...
Widevine 라이선스 토큰 인터페이스는 프로덕션 및 테스트 서비스를 제공합니다.

이 HTTP 요청은 Widevine 라이선스로 상환될 수 있는 토큰을 반환합니다.
...
<table id="table_ww1_hcs_pv">  
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> html <span class="codeph"> 또는 </span> json <span class="codeph"> </span>중 하나. <p>html(기본값) <span class="codeph"> </span> 이 응답의 엔티티 본문에 오류의 HTML 표현을 제공하는 경우 json을 <span class="codeph"> </span> 지정하면 JSON 형식의 구조화된 응답이 반환됩니다. 자세한 내용은 <a href="https://www.expressplay.com/developer/restapi/#json-errors" format="html" scope="external"> JSON 오류를 </a> 참조하십시오. </p> <p>MIME 유형은 성공 시 <span class="codeph"> 텍스트/uri-list, html </span> 형식의 텍스트/html, JSON 오류 포맷의 <span class="codeph"> JSON, JSON 오류 포맷의 </span> <span class="codeph"> </span> <span class="codeph"> </span> <span class="codeph"> </span> MIME/URL 중 하나입니다. </p> </td> 
   <td> 아니요 </td> 
  </tr> 
 </tbody> 
</table>
...
| `kid` | 컨텐츠 암호화 키 또는 문자열의 16바이트 16진수 문자열 표현 `^somestring'`. 문자열 뒤에 오는 길이는 64자보다 길 수 `^` 없습니다. 예를 보려면 아래 메모를 확인하십시오. | 예 |
| `ek` | 암호화된 콘텐츠 키의 16진수 문자열 표현입니다. | 아니요 |
| `contentKey` | 컨텐츠 암호화 키의 16바이트 16진수 문자열 표현 | 예, `kek` 및 `ek` 또는 제공되지 `kid` 않는 한 |
...
| `licenseDuration` * | 라이선스 기간(초)입니다. 제공되지 않으면 기간에 제한이 없음을 나타냅니다. 자세한 내용은 아래 메모를 참조하십시오. | 아니요 |
| `wvExtension` | 짧은 양식 래핑 확장자Type 및 extensionPayload를 쉼표로 구분된 문자열로 나타냅니다. 아래 형식을 참조하십시오. 예: `…&wvExtension=wudo,AAAAAA==&…` | 아니요, 아무 번호나 사용할 수 있습니다 |

정보 `licenseDuration`: <ol><li> 재생을 시작한 후 `licenseDuration` 몇 초 후에 재생이 중지됩니다. </li><li> 제한 없는 시간 동안 재생을 중지/다시 시작하려면 `licenseDuration` (기본적으로 무한) 생략합니다. 그렇지 않으면 최종 사용자가 스트림을 즐길 수 있는 시간을 지정합니다. </li></ol>

**표 15:토큰 제한 쿼리 매개 변수**

| 쿼리 매개 변수 | 설명 | 필수 |
|--- |--- |--- |
| `expirationTime` | 이 토큰의 만료 시간입니다. 이 값은 RFC 3339 [날짜/시간](https://www.ietf.org/rfc/rfc3339.txt) 형식의 &#39;Z&#39; 영역 지정자(&quot;Zulu time&quot;)에 있는 문자열이나 + 기호 앞에 오는 정수여야 합니다. RFC 3339 날짜/시간의 예는 2006-04-14T12:01:10Z입니다. <br> 값이 RFC 3339 [날짜](https://www.ietf.org/rfc/rfc3339.txt) /시간 형식의 문자열이면 토큰의 절대 만료 날짜/시간을 나타냅니다. 값이 앞에 + 기호가 있는 정수인 경우 발행부터 해당 토큰이 유효하다는 상대 시간(초)으로 해석됩니다. 예를 들어 `+60` 1분을 지정합니다. <br> 최대 및 기본(지정되지 않은 경우) 토큰 수명은 30일입니다. | 아니요 |

**표 16:상관 관계 쿼리 매개 변수**

| **쿼리 매개 변수** | **설명** | **필수** |
|---|---|---|
| `cookie` | 토큰에 최대 32자까지 포함되고 토큰 상환 서버에서 로깅되는 임의의 문자열입니다. 상환 서버와 서비스 공급자의 서버에 있는 로그 항목의 상관 관계를 지정할 수 있습니다. | 아니요 |
...
<table id="table_agj_gqx_pv">  
 ...
 <tbody> 
  ...
  <tr> 
   <td> -2018 </td> 
   <td> 인증 토큰이 잘못되었습니다.&lt;details&gt; <p>참고: 인증자가 잘못된 경우 또는 *.test.express.com에서 프로덕션 인증자를 사용하여 테스트 API에 액세스할 때 또는 그 반대로 발생할 수 있습니다. </p> <p importance="high">참고: 테스트 SDK 및 고급 테스트 도구(ATT)는 <span class="filepath"> *.test.expressplay.com에서만 작동하지만 프로덕션 장치는 </span><span class="filepath"> *.service.express.com을 사용해야 합니다. </span> </p>. </td> 
  </tr> 
  <tr> 
   <td> -2019 </td> 
  ...
  <tr> 
   <td> -4018 </td> 
   <td> 실종된 <span class="filepath"> 아이 </span> </td> 
  </tr> 
  <tr> 
   <td> -4019 </td> 
  ...
  <tr> 
   <td> -7002 </td> 
   <td> 장치 ID 바인딩이 Widevine에 대해 지원되지 않음 </td> 
  </tr> 
  <tr> 
   <td> -7003 </td> 
   <td> 보안 수준 값이 없습니다. </td> 
