...
# FairPlay를 위한 멀티 DRM 워크플로우 {#multi-drm-workflow-for-fairplay}
...
1. Apple에서 자격 증명을 얻습니다.
   ...
   >[!NOTE]
   >
   >기본 역할 **[!UICONTROL Content Provider]** 에 대해 선택합니다.

   요청이 승인되면 Apple은 *FairPlay 스트리밍 배포 패키지를 전송합니다*.
1. 인증서 서명 요청을 생성합니다.

   공개/개인 키 쌍 [!DNL openssl] 과 인증서 서명 요청(CSR)을 생성하는 데 사용할 수 있습니다.
   ...
   1. CSR 생성
      ...
      >[!NOTE]
      >
      >이 단계에 대한 지침은 *FairPlay 스트리밍 배포 패키지에*&#x200B;있지만 사용자의 편의를 위해 여기에 포함됩니다. 이 프로세스에 문제가 있는 경우 배포 패키지에서 *FairPlayCertificateCreation.pdf* 지침을 확인하십시오.

1. Apple 개발자 포털을 통해 CSR을 업로드합니다.
   1. 개발 팀의 팀 에이전트가 로그인해야 합니다 [!DNL developer.apple.com/account].
   1. 을 **[!UICONTROL Certificates, Identifiers & Profiles]**&#x200B;클릭하고 페이지의 왼쪽 상단에 있는 **[!UICONTROL iOS, tvOS, watchOS]** **[!UICONTROL Certificates->Production]** 드롭다운을 선택한 다음 페이지의 왼쪽에 있는 아이콘을 클릭합니다.
   ...
   1. iOS *인증서*&#x200B;추가에서 2.b. 단계에서 생성한 CSR 파일을 업로드하고 을 클릭합니다 **[!UICONTROL Generate]**.

      ASK(응용 프로그램 비밀 키)가 동일한 대화 상자에 표시됩니다.
   1. ASK를 적어서 안전한 장소에 보관하세요
   1. ASK에서 키를 눌러 인증서 생성을 완료하고 을 클릭합니다 **[!UICONTROL Continue]**.
   1. ASK를 저장했는지 확인한 후 계속하려면 을 클릭하십시오 **[!UICONTROL Generate]** .
...
1. FairPlay 자격 증명으로 ExpressPlay 계정을 설정합니다.
   1. 3.h 단계에서 다운로드한 인증서 이름을 살펴보겠습니다.은(는) [!DNL fairplay.cer]으로 설정되어 있습니다.
   1. Apple Keychain Access 유틸리티로 [!DNL fairplay.cer] 파일을 엽니다.
   1. 오른쪽 상단에 있는 검색 필드에 &quot;&quot; `fairplay`&quot;를 입력하여 많은 인증서를 필터링합니다.
   1. 회사의 FairPlay 인증서를 확인합니다.

      회사 이름은 Apple에서 발행한 인증서와 연결해야 합니다.
   1. 확장 화살표를 선택하여 인증서를 확장하고 개인 키를 마우스 오른쪽 단추로 클릭합니다.
   1. 파일 **[!UICONTROL Export "Your Company Name"]** 을 선택하고 [!DNL .p12] 저장합니다.
   ...
   1. 왼쪽 위 **[!UICONTROL DRM SERVICES]** 를 클릭하고 **[!UICONTROL FairPlay]** 탭을 선택합니다.
...
이제 FairPlay용 ExpressPlay 서비스를 사용하여 인증서와 함께 FairPlay 컨텐츠 보호 기능을 사용하여 iOS 애플리케이션 또는 HTML5 페이지를 만들 수 있습니다. [!DNL fairplay.cer]

<!--<a id="fig_sjr_2pn_sv"></a>-->

![](assets/multi_drm_expressplay_drm_services_web.png)

### FairPlay용 컨텐츠 패키지 {#package-your-content-for-fairplay}
...
1. 콘텐츠 패키징
   ...
   다음은 Adobe Offline Packager를 사용하는 패키지 예입니다. Packager에서는 다음과 같은 구성 파일(예: [!DNL fairplay.xml]구성 파일)을 사용합니다.
   ...
   * `drm_sys` - 패키징하는 DRM 솔루션 이 `FAIRPLAY` 경우에 있습니다.
   * `frag_dur` - 조각 지속 시간(초)
   ...
   * `key_url` - 파일 태그의 URI 매개 `EXT-X-KEY` [!DNL .m3u8] 변수입니다.
   * `content_id` - 기본값

   Packager 설명서에 명시된 [대로](https://helpx.adobe.com/content/dam/help/en/primetime/guides/offline_packager_getting_started.pdf#page=7), &quot;최상의 방법은 출력 생성에 사용하고자 하는 일반적인 옵션이 포함된 구성 파일을 만드는 것입니다. 그런 다음 특정 옵션을 명령줄 인수로 제공하여 출력물을 만듭니다.&quot;
   ...
   생성된 M3U8 파일에는 다음과 같은 `EXT-X-KEY` 속성이 있습니다.
...
Adobe은 *시간 기반* 및 *장치 바인딩* 권한 부여를 수행하는 방법을 보여주는 샘플 ExpressPlay 자격 부여 서버(SEES)를 제공합니다. 이 샘플 권한 부여 서버는 ExpressPlay 서비스 위에 구축되어 있습니다.
...
iOS TVSDK 클라이언트에서 라이선스 및 재생을 구현하는 방법은 다음과 같습니다. [TVSDK 애플리케이션에서 Apple FairPlay를 사용할 수 있습니다](../../../programming/tvsdk-3x-ios-prog/ios-3x-drm-content-security/ios-3x-apple-fairplay-tvsdk.md). 또한 FairPlay에 대한 오프라인 재생 및 라이선스 회전을 선택적으로 구현할 수 있습니다.
...
이 작업을 시작하기 전에 Apple 문서 **&quot;Offline Playback with FairPlay Streaming 및 HTTP Live Streaming&quot;을 다운로드하여 읽어 보십시오**. TS(전송 스트림) 세그먼트를 다운로드하고 로컬 시스템에 저장하는 방법을 알아보려면 이 안내서를 읽어 보십시오.

이 워크플로우에서 FairPlay에 대한 오프라인 재생을 구현합니다.

1. HLS TS 세그먼트를 다운로드합니다.
1. FairPlay 서버에서 영구 대여 라이선스 신청( **&quot;FairPlay 영구 임대 정책&quot; 참조**)
...
>
>자세한 [내용은 HTTP 라이브 스트리밍](https://developer.apple.com/library/content/documentation/AudioVideo/Conceptual/MediaPlaybackGuide/Contents/Resources/en.lproj/HTTPLiveStreaming/HTTPLiveStreaming.html#//apple_ref/doc/uid/TP40016757-CH11-SW3) 문서 작업을 참조하십시오.
