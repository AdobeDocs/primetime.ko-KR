...
이 문제는 AUDITUDE_METADATA_KEY 를 확인하는 코드를 추가하고 AUDITUDE_METADATA_KEY 와 ADVERTISING_METADATA_KEY 를 상호 교환 가능하게 하여 해결되었습니다.
...
광고 요청에 대한 광고 단위를 만들 때 감사 요청은 사용자 지정 매개 변수에 대한 AuditudeSettings를 구문 분석합니다.

이 동작은 Opportunity 객체의 사용자 지정 매개 변수를 요청에 포함하도록 변경되었습니다. 또한, 서로 다른 사용자 지정 매개 변수를 사용하는 여러 기회를 하나의 감사 요청에 채울 수 없습니다.
...
* **버전 1.4.0**
   ...
   * 다른 도메인의 콘텐츠를 스트리밍할 수 있는 권한에는 도메인 간 정책 파일(crossdomain.xml)이 필요합니다. [HTTP 스트리밍을 위한 crossdomain.xml 파일 설정](https://www.adobe.com/devnet/adobe-media-server/articles/cross-domain-xml-for-streaming.html).
