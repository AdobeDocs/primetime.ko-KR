...
Android용 TVSDK는 이전 버전보다 향상된 많은 성능을 제공합니다. 고품질의 시청 경험을 제공하고 다중 CDN 지원을 제외한 버전 1.4의 모든 기능을 전달합니다.
...
* **대역폭 속도가 갑자기 끊기는 경우에도 최고의 재생 경험을 얻을 수 있도록 최적화되었습니다.**
...
* **매끄러운 트릭 플레이 -** 이러한 향상된 기능을 통해 사용자 경험을 향상시킬 수 있습니다.
   ...
   * 트릭 재생 중에 대역폭 및 버퍼 프로필을 기반으로 적응형 비트 전송률과 프레임 속도를 선택할 수 있습니다
...
* **SizeAvalableEventListener**
   ...
   * `getHeight()` 및 `getWidth()` 메서드 `SizeAvailableEvent` 는 이제 각각 높이와 폭으로 출력을 반환합니다. 디스플레이 종횡비는 다음과 같이 계산할 수 있습니다.
      ...
      Sar 너비 및 Sar 높이 측면에서 저장소 종횡비를 사용하여 프레임 폭과 프레임 높이를 계산할 수도 있습니다.
...
* ZD#37808 - 목록 새 사용자 지정 헤더 허용
   * SSAI_TAG 지원이 이 픽스의 일부로 추가되었습니다.
* ZD#37622 - URIS특정 광고 창에서 세금 오류 동기화
   * 고객 Android 앱이 인코딩되지 않은 % 광고를 제공받을 때 스트림 재생 중단과 관련된 문제가 해결되었습니다.
* ZD#37631 - Android TVSDK용 마스터 매니페스트 재시도 메커니즘
   * 이 개선 사항을 처리하기 위해 네트워크 구성에 새 API를 추가했습니다. 이 API를 사용하지 않으면 매니페스트가 다시 시도되지 않습니다. 사용하는 경우 네트워크 오류 및 시간 초과를 처리하기 위해 매니페스트를 다시 시도합니다.

**버전 3.2**

* ZD#37493- 실시간 재생을 위한 추적 비콘이 시퀀스의 첫 번째 광고에 대해 간헐적으로 실행되지 않습니다.
* ZD#36985- VMAP 응답에서 빈 광고 중단에 대해 추적 비콘이 전송되지 않습니다.
* ZD#37134 - TVSDK가 간헐적으로 VMAP 응답에 대해 잘못된 ID를 던집니다.

**버전 3.0**

* ZD#33740 - TVSDK에서 MediaPlayer 개체를 만들고 replaceCurrentResource()를 호출한 직후에 불필요한 경고가 표시됩니다.

   * 플레이어가 일시 중단 상태인 경우에만 복원을 호출하여 이전 수정 사항을 개선했습니다.

* ZD#36442 - 모든 새로운 재생을 통해 원격 디버깅 세션에 연결이 끊기므로 디버깅을 수행할 수 없습니다.

   * 디버깅이 기본적으로 활성화되어 있지 않으므로 웹 보기에서 디버깅을 기본적으로 수행할 수 없습니다. MediaPlayer.getCustomAdView()에서 반환된 개체에서 setWebContentsDebuggingEnabled(true)를 호출하여 필요한 경우 앱을 활성화해야 합니다.

* ZD#33688 - 적시에 지원 및 해결

   * 광고 중단은 광고 중단의 위치 전에 지정된 간격으로 해결됩니다.

* ZD#36441 - 라이브 창의 지속 시간이 5분을 넘어 계속 증가하여 여러 문제가 발생합니다.
...
* ZD #33062 - CDATA 노드의 VAST 응답에서 파이프 문자가 발생하는 경우 TVSDK 충돌
   ...
   * NetworkConfiguration 클래스의 API setEncodeUrlForTracking이 인코딩될 URL의 안전하지 않은 문자로 제거됨
...
* ZD #33619 - 라이브 지점 근처에 버퍼링 상태가 있는 증가하는 재생 목록 콘텐츠를 빠르게 전달합니다.
   ...
   * 트릭 재생 모드에서 라이브 포인트를 교차할 때 이 문제를 해결했습니다.
...
* Zendesk#31806 - Auditude sticing

   응답 xml에 빈 태그가 있기 때문에 플레이어가 준비 중입니다. 이제 문제가 해결되었습니다.
...
* Android에서 재생되지 않는 Zendesk#32794-1080P 해상도 스트림
   ...
   미디어 형식에서 반환되는 프레임 높이 및 프레임 너비를 반환하는 데 사용되는 2.5에서 SizeAvailableEvent 및 Previous, getHeight() 및 getWidth() 메서드를 변경했습니다. 이제 디코더가 각각 반환하는 출력 높이와 출력 너비를 반환합니다.
...
* CMAF(CBC) 스트림에 대해 ID3, 자막, 지연 바인딩 오디오 지원이 확인되지 않았습니다.
* 일부 장치에서는 CMAF 스트림에서 트릭 재생 중 상단에 비디오 왜곡이 나타날 수 있기 때문에 재생성이 낮은 문제가 발생합니다.
