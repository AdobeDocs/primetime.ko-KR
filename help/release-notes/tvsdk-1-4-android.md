...
* **온-프레미스 개인화**&#x200B;지원다른 종단점으로 이동하기 위한 클라이언트의 개인화 요청을 사용자 지정하기 위해 Adobe Indification Server의 온-프레미스 설치를 지원합니다.
...
* Zendesk #28976 - 라이선스 요청을 완료하는 데 1초 이상이 소요됩니다. POST을 사용하는 DRM 라이센스 요청 호출이 실행 중일 때 Curl은 &quot;예상:100-continue&quot; 헤더. TVSDK에서 &quot;예상:&quot; 헤더가 제거되었습니다.
...
* Zendesk #26551 - CRS 오류 수정
   * 요청이 HEAD(http head)인 경우 비어 있으므로 컨텐츠를 읽을 필요가 없습니다. 읽어도 괜찮지만 read()를 호출하면 이전 Android(4.0.x)가 정지되고, 읽기()를 호출하면 최신 Android 반환 올바른 값(-1)이 반환됩니다. 이에 따라 코드가 &quot;head&quot;에 대한 컨텐츠를 읽지 않도록 변경되었습니다.
...
* Zendesk #2839 - Adobe Primetime PSDK가 ARMv8 칩셋을 지원합니까? 갤럭시 S6에서 발견된 충돌사고에 대한 수정 사항을 추가했습니다.
...
* PTPLAY-5084 - 라이브 매니페스트 업데이트 기본 장애 조치(failover) 지원
...
* Zendesk #2256 - Access to 기본 Playlist, updated PSDK to dispatch timedMetadata events for subscribed tags on the master playlist.
