...
## Primetime Streaming Server 1.4(12월 릴리스)의 새로운 기능 {#what-s-new-in-primetime-streaming-server-december-release}
...
## Primetime Streaming Server 1.3.1(5월 릴리스) {#what-s-new-in-primetime-streaming-server-may-release}의 새로운 기능
...
* 3759167 - 일부 SCTE35 큐를 사용하면 패키징 중 타임스탬프 이상 문제가 발생하여 출력 매니페스트에 도달하지 않습니다. SCTE35 메시지의 SpliceInfoSection의 TimeSignal에서 SpliceTime에 pts_adjustment를 적용합니다.
...
1. [Oracle 사이트](https://www.oracle.com/technetwork/java/javase/downloads/index.html)에서 Java SE 및 JDK 소프트웨어를 다운로드하고 설치 지침을 따르십시오.
2. Adobe Primetime 스트리밍 서버 1.4 아카이브 파일 `Primetime- StreamingServer-1-4-0-b206-12042014.zip`을(를) 디스크에 추출합니다.
...
1. [Oracle 사이트](https://www.oracle.com/technetwork/java/javase/downloads/index.html)에서 Java JRE 및 JDK 소프트웨어를 다운로드하고 설치 지침을 따르십시오.
...
**Live Packager를** 시작하려면 Packager의 루트 디렉토리에서 다음 명령을 실행합니다.\
`$packager_start.sh`
...
Adobe Primetime - Live Packager 1.4 아카이브를 추출한 후, 디렉토리를 Packager 디렉토리로 변경하고 packager_start.sh 스크립트를 실행합니다. 샘플 구성은 멀티캐스트 주소 239.235.0.3:14000에서 수신 대기하며 포트 8080에서 로컬 원본 서버를 실행합니다. 출력은 `packager/webroot/_default_/_default_/ directory`에 쓰도록 구성됩니다.
...
1. Adobe Primetime - Offline Packager 1.4 아카이브 파일 `Primetime- OfflinePackager-1-4-0-b206-12042014.zip`을(를) 디스크에 추출합니다.

[여기](https://help.adobe.com/en_US/primetime/api/packagers/offline/index.html)에서 사용 가능한 구성 세부 사항은 Primetime Offline Packager 시작 문서를 참조하십시오.
