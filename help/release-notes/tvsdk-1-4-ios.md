...
seo-description: Android용 TVSDK 1.4 릴리스 정보에서는 TVSDK iOS 1.4의 새로운 기능 또는 변경된 기능, 해결되고 알려진 문제 및 디바이스 문제를 설명합니다
...
* **allowExternalRecording**:보안 컨텐츠에 대한 에어플레이 미러링을 허용하는 새로운 API가 도입되었습니다. Airplay 미러링은 레코딩으로 처리되므로 allowExternalRecording 값을 &#39;True&#39;로 설정해야 합니다. 이렇게 설정하면 에어플레이 미러링을 허용하거나 &#39;False&#39;로 설정하여 보안 컨텐츠의 에어플레이 미러링을 중지할 수 있습니다. 기본값은 true입니다.
...
PTSDKConfig 클래스는 Adobe Primetime 광고 결정, DRM 및 비디오 분석 서버에 대한 요청에 SSL을 적용하는 방법을 제공합니다. 자세한 내용은 이 클래스의 `forceHTTPS` 및 `isForcingHTTPS` 메서드를 참조하십시오. 매니페스트가 HTTPS를 통해 로드되는 경우 TVSDK는 HTTPS의 컨텐츠 사용을 보존하며 해당 매니페스트에서 상대 URL을 로드할 때 이러한 사용을 반영합니다.

**참고**:광고 추적 픽셀, 컨텐츠 및 광고 URL 및 유사한 요청과 같은 타사 도메인에 대한 요청은 수정되지 않으며, HTTPS를 통해 지원되는 URL을 제공하는 것은 컨텐츠 제공업체 및 광고 서버의 책임입니다.

**버전 1.4.18**

Primetime iOS TVSDK는 이제 VPAID 2.0 Javascript 크리에이티브를 지원하므로 인터랙티브한 리치 인스트림 광고 경험을 제공할 수 있습니다.
...
**참고**:TVSDK 빌드에서 Nielsen 모듈이 제거되었고, TVSDK가 곧 새로운 Nielsen 통합 모듈로 업데이트될 예정입니다.
...
자세한 내용은 [VAST 및 VMAP 광고 대비 대체(a1/>를 참조하십시오.](../programming/tvsdk-1.4-for-ios/ad-insertion/c-psdk-ios-1.4-ad-fallback.md)
...
이제 장치의 출력 보호 기능에 따라 DRM 정책이 허용된 최고 해상도를 지정할 수 있습니다. 예를 들어 &quot;HDCP를 사용할 수 있는 경우 최대 1080p 해상도의 컨텐츠를 재생할 수 있으며 HDCP를 사용할 수 없는 경우 최대 480p 해상도의 컨텐츠를 재생할 수 있습니다.&quot;
...
* **Nielsen SDK 통합**
   ...
   * 이제 TVSDK는 사용자 정의 통합 없이 mTVR 및 MDPR ID3 비콘 전송을 Nielsen SDK로 지원합니다. 시작하려면 3.1.2.19 Nielsen iOS 앱 SDK를 다운로드하십시오.
...
* 티켓 번호 36294 - Xcode 10에서 iOS TVSDK가 작동하지 않음
...
* 일시 중지된 상태 동안 새 위치를 찾을 때 티켓 번호 34801 - currentTime 및 localTime이 업데이트되지 않았습니다.
   ...
   * 플레이어가 일시 중지된 상태에서 플레이어의 현재 시간을 0으로 설정할 수 있습니다.이전에는 재생 상태에서만 0으로 설정되었던 현재 시간이 이전이었습니다.

* 티켓 번호 35037 - 신호 기반 광고 삽입에서 돌아올 때 재생이 잘못된 URL을 정지합니다.
...
* (ZD#34385) - 신호 기반 광고 삽입에서 돌아올 때 재생이 잘못된 URL을 표시합니다.
   ...
   매니페스트 읽기가 계속 실행될 수 있도록 CustomAVAssetLoaderOperations에 대한 최대 동시 카운트를 늘립니다.
...
* (ZD#33904) - TVSDK가 AVFfoundation 알림 AVAudioSessionMediaServicesWasLostNotification 및 AVAudioSessionMediaServicesWasResetNotification에 등록되어 있지 않습니다.

   이제 미디어 서비스가 재설정되거나 손실될 때 알림을 받기 위해 플레이어 앱에 PTMediaServicesWereLostNotification 및 PTMediaServicesWasResetNotification을 등록할 수 있습니다.
...
* (ZD #32465) - 플레이어에서 병합된 재생 목록을 처리할 수 없습니다.
   ...
   Call finishLoadingWithError(with:오류) AV 재단이 대체 스트림/트리거 장애 조치를 시도하려면.
...
* (ZD #3336) - [iOS] TVSDK - Freewheel에서 반환되는 충분한 광고에도 불구하고 광고 창이 채워지지 않습니다.
...
* (ZD# 27460) 미드롤 첫 광고 통화 - 403을 반환하는 cdn.auditude<span></span>.com으로의 POST
...
이 문제는 Live/Linear 스트림의 자산 지속 시간을 업데이트하기 위해 assetDuration API를 `PTVideoAnalyticsTrackingMetadata`에 추가하여 라이브 스트림을 확인하는 논리를 제공하여 해결되었습니다.
...
* (ZD #19460) - 충돌 - [PTMediaPlayerItem prepareSubjectOptionsWithAVMediaSelectionOptions:nonForcedOptions:]
...
TVSDK 라이브러리가 YUV -> RGB 색상 변환을 수행하는 네온 코드를 사용할 때 TVSDK가 Android M OS에서 작동 중이었습니다. 이 문제는 네온되지 않은 코드 버전을 사용하여 이 문제를 일으키는 함수를 업데이트하여 해결되었습니다.
...
빈 광고 브레이크 추적 URL을 트리거하는 지원, TVSDK는 이제 빈 광고 중단에 대한 광고 중단 시작 및 완료 페이지를 확인합니다
...
* (ZD #2141) TreeHouse 앱에 대한 분석 구현에서 패키지를 빌드할 AdobeAnalyticsPlugin.a 라이브러리를 추가했습니다.
...
* 최소 타겟이 iOS7(이상)로 설정된 경우 drmNativeInterface.framework가 올바르게 링크되지 않습니다.\
   해결 방법:`libstdc++6`을(를) 명시적으로 지정합니다.  dylib 라이브러리는 다음과 같습니다.Target->빌드 단계->바이너리를 라이브러리와 연결하고 `libstdc++.6.dylib`을(를) 추가합니다.
