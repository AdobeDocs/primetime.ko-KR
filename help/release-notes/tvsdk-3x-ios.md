...
# iOS용 TVSDK 3.11 릴리스 노트 {#tvsdk-for-ios-release-notes}

iOS용 TVSDK 3.11 릴리스 노트는 TVSDK iOS 3.11의 새로운 기능 또는 변경된 사항, 해결되고 알려진 문제 및 장치 문제를 설명합니다.

## 시스템 및 소프트웨어 요구 사항 {#system-software-requirements}

iOS 3.11을 다운로드하기 전에 하드웨어, 운영 체제 및 응용 프로그램 버전이 다음 요구 사항을 충족하는지 확인하십시오.

운영 체제:iOS 8.0 이상.

## iOS TVSDK 3.11

애플리케이션 충돌을 일으키는 `isFallbackOnInvalidCreativeEnabled` 위치 및 방법에 `customParams` 대한 수정 사항을 제공합니다.

현재 릴리스의 수정 사항은 [고객 문제가](#resolved-issues) 수정되었으며 제한 사항은 [알려진 문제 및 제한](#known-issues-and-limitations) 섹션을 참조하십시오.
...
핫픽스는 업데이트된 개별화 전송 인증서를 가지고 있었습니다.

**iOS TVSDK 3.8**

iOS 13 규정 준수 및 iOS 13 UIWebView API 사용 중단 문제를 처리했습니다.

**iOS TVSDK 3.7**

많은 수의 광고 해상도 요청이 동시에 수행되었을 때 재생이 중지되는 시나리오에 대한 핫픽스입니다.
...
이 기능을 사용하려면 PTMediaPlayer 클래스에 `audioPlaybackInBackground` 추가된 새 API를 설정해야 합니다. 이 API를 활성화하면 앱이 백그라운드 오디오를 재생할 수 있습니다.

**iOS TVSDK 3.4.0.19 (핫픽스)**

이 릴리스에는 광고 장애 조치 시나리오에서 발생하는 애플리케이션 충돌에 대한 수정 사항이 있습니다.

**iOS TVSDK 3.4**

**광고 해상도 시간 초과**

* 이제 사용자는 TVSDK 3.4를 사용하여 전체 광고 해상도 및 매니페스트 다운로드에 대한 시간 초과 값을 설정할 수 있습니다. 주어진 시간 제한 내에 일부 광고가 해결되지 않으면 TVSDK가 나머지 광고를 재생합니다.

* PTAdMetadata:adRequestTimeout API는 더 이상 사용되지 않으며 제거됩니다. 기본값은 35초로 설정되었습니다.

* 두 개의 새로운 대체 API가 PTAdMetadataClass에 도입되었습니다.adResolutionTimeout - 전체 광고 해상도 호출에 대한 시간 초과 및 adManifestTimeout - 광고 매니페스트 다운로드에 대한 시간 초과

**수익 최적화**

TVSDK를 사용하여 광고 삽입 워크플로우와 관련된 문제 영역을 식별하여 분석 종단점에 보고할 수 있습니다.

**버전 3.3**

TVSDK 3.3은 이제 iOS 11 SDK와 호환됩니다. 더 이상 사용되지 않는 모든 API가 적합한 대체 항목으로 대체되었습니다.
...
* **이제 AVC 코덱을 재생하는** Fairplay 암호화된 CMAF 스트림 지원이 추가되었습니다.
...
JIT를 사용하도록 앱 수준 인터페이스에 부울 유형의 `enableDelayAdLoading` 속성을 추가했습니다. NO가 `enableDelayAdLoading` NO이면 True(PTAdMetadata 인터페이스의 속성) `setadMetadata.delayAdLoading`로 설정됩니다.

이 속성을 활성화하면 TVSDK는 정의된 허용치 값을 기준으로 해당 위치 이전의 각 광고 브레이크를 해결합니다. 기본적으로 5초로 `delayAdTolerance` 설정됩니다.

**버전 1.4.45**

Xcode10을 준수하기 위해 TVSDK는 &quot;`libstdc++`&quot;에서 &quot;`libc++`&quot;로 전환되었으며, 그 결과 iOS 7에서 지원되는 최소 버전이 되었습니다. 이전에는 iOS 6이었습니다.
...
이 릴리스에는 새로운 기능이 추가되지 않았습니다. 해결된 문제 목록은 해결된 [문제를](#resolved-issues)참조하십시오.
...
* **allowExternalRecording**:보안 컨텐츠에 대해 에어플레이 미러링을 허용하도록 새로운 API가 도입되었습니다. Airplay 미러링은 레코딩으로 처리되므로 `allowExternalRecording` 값을 `True`설정하여 에어플레이 미러링을 허용하거나 보안 컨텐츠를 위해 에어플레이 미러링을 `False` 중지하도록 설정해야 합니다. 기본적으로 `value` true입니다.
...
iOS TVSDK에서 VHL 2.0 통합 및 인증:API의 복잡성을 줄여 `VideoHeartbeatsLibrary` 구현 장벽을 줄일 수 있습니다.
...
* **CRS Ads TVSDK에 대한 다중 CDN** 지원은 이제 CRS 광고에 대한 다중 CDN을 지원합니다. CRS 광고에 대한 FTP 세부 정보를 제공하여 Akamai와 같은 기본 Adobe 소유 CDN 이외의 CDN 위치를 지정할 수 있습니다.

**버전 1.4.29**

클래스에서 forceHTTPS API가 `PTSDKConfig` 추가되었습니다.
...
Primetime iOS TVSDK는 VPAID 2.0 Javascript 크리에이티브를 지원하므로 인터랙티브한 리치 인스트림 광고 경험을 제공할 수 있습니다. VPAID 2.0에 대한 자세한 내용은 VPAID 광고 지원을 참조하십시오.
...
* 비디오 시작 및/또는 비디오/광고/장 시작에서 컨텍스트 데이터로 메타데이터를 전송할 수 있습니다.
...
이제 DRM 정책은 장치의 출력 보호 기능에 따라 허용되는 최고 해상도를 지정할 수 있습니다. 예를 들어 &quot;HDCP를 사용할 수 있는 경우 최대 1080p 해상도의 콘텐트를 재생할 수 있고 HDCP를 사용할 수 없는 경우 최대 480p 해상도의 콘텐트를 재생할 수 있습니다.&quot;
...
이제 TVSDK는 사용자 정의 통합 없이 mTVR 및 MDPR ID3 비콘을 Nielsen SDK로 전송할 수 있습니다. 시작하려면 3.1.2.19 Nielsen iOS 앱 SDK를 다운로드하고 iOS 프로그래머 가이드에 있는 지침을 따르십시오.
...
* (ZD 파섹#40998) - `isFallbackOnInvalidCreativeEnabled` 애플리케이션이 충돌할 수 있습니다.

* (ZD 파섹#41289) - `NSInvalidArgumentException` 애플리케이션 `customParams` 충돌을 유도하는 방법과 함께 관찰됩니다.

### 이전 릴리스의 해결된 문제 {#resolved-issues-previous}

**iOS TVSDK 3.10**

(ZD#40943) - TVSDK 플레이어에서 네트워크를 사용할 수 없을 때 PTMediaPlayerStatus오류 알림을 실행하지 않습니다.

**iOS TVSDK 3.9**

(ZD#40272) - iOS TVSDK에서 101001 오류가 발생하여 VTT 자막을 재생하지 못해 앱이 중단됩니다.

**iOS TVSDK 3.8**

* (ZD 파섹#40087) - 만료된 VOD 컨텐츠에 대한 플레이어 오류로 iOS가 충돌합니다.

* (ZD 파섹#40083) - 프리롤 광고는 라이브 스트림에 대해 재생되지 `OpportunityGenerator` 않으며 플레이어에서 오류가 발생합니다.

* (ZD#39828) - `CurrentItem` 속성에 Null 허용 주석을 누락하여 알림에 포함된 플레이어 상태가 있으면 플레이어 충돌이 발생합니다 `PTMediaPlayerStatusStopped`.

**iOS TVSDK 3.7**

(ZD#38961) - PiP에서 재생되도록 여러 콘텐트를 구성한 경우 한 컨텐츠가 재생되는 것을 완료한 후 PiP(Picture-in-Picture) 창에서 콘텐트가 재생되지 않습니다.
...
* **Ticket#36588** - MediaPlayer STOP 메서드가 호출될 때 플레이어 충돌이 발생합니다.
자막이 있는 몇 개의 스트림에 대해 STOP 메서드를 호출할 때 간헐적으로 발생하는 충돌 문제가 수정되었습니다.

* **티켓#37080** - 매니페스트 호출에 대해 중복된 요청이 표시되었습니다.
재생 중에 매니페스트 URL에 대해 수행된 중복 요청이 수정되었습니다. 이제 TVSDK에서 매니페스트당 한 번 호출합니다.

* **티켓#37** - CRS 정규화 규칙이 eq 일치 유형으로 실패합니다. 플레이어에서 &quot;eq&quot; 일치 유형이 있는 호스트 이름에 대한 마지막 정규화 규칙 집합에서 발생하는 경우 충돌이 발생하는 문제가 수정되었습니다.

**버전 3.1**

**티켓 번호36313** - 선형 광고 중단 중 간헐적으로 예측할 수 없는 결과라이브 스트림의 선형 광고 중단 중 간헐적인 재생이 수정되었습니다.

**버전 3.0.1**

**Ticket36948** - CRS - iOS 12에서 자산 선택 순서가 일치하지 않습니다. CRS에 대해 선택된 자산이 VAST 또는 VMAP 응답에서 반환되는 최고 품질 변형이 항상 아닌 경우

**버전 3.0**

* **Ticket35311** - 전화 통화 중단 동안 플레이어 상태가 일시 중지되지 않음플레이어 중단을 중단하도록 인터럽트 핸들러를 추가했습니다. 중단하면 플레이어 상태가 일시 중지됨이 되고 재생 단추를 클릭하면 재생을 다시 시작합니다.

* **Ticket36685** - Live assets - Time mismatch with player time progress, and SCTE marker timeCorrect time is calculated for the SCTE markers that are ahead of live point.

* **Ticket36492** - `currentTime` 일시 중지된 상태 동안 새 위치를 찾을 때 업데이트되지 `localTime` 않음Player가 일시 중지 상태인 경우 현재 시간을 0으로 설정할 수 있습니다.이전에는 재생 상태에서만 0으로 설정되었던 현재 시간을 초과했습니다.

**버전 1.4.45**

* **Ticket36294** - iOS TVSDK가 Xcode 10에서 작동하지 않음 XCode 10에서 TVSDK의 컴파일 문제가 해결되었습니다. XCode 10 요구 사항으로 인해 iOS 1.4.45 이상 버전용 TVSDK에서 앱을 빌드하려면 iOS 7.0과 같은 최소 배포 대상이 필요합니다.

* **Ticket36321** - &quot;재생&quot; 상태에서 `PTMediaPlayer` `AVPlayer` 인스턴스와 인스턴스 사이의 검색 가능한 범위에서 불일치가 관찰되었습니다.

* **Ticket36493** - iOS 12에서 `libstdc++` 지원 iOS 12에서 TVSDK의 컴파일 문제가 해결되었습니다. iOS 1.4.45용 TVSDK를 기반으로 구축된 앱은 iOS 7.0과 같이 최소 배포 대상이 필요합니다.
...
* **Ticket34801** - 일시 중지된 상태 동안 새 위치를 찾을 때 CurrentTime 및 localTime이 업데이트되지 않았습니다. 이제 플레이어가 일시 중지 상태인 경우 Player의 현재 시간을 0으로 설정할 수 있습니다.이전에는 재생 상태에서만 0으로 설정되었던 현재 시간을 초과했습니다.

* **Ticket35037** - 신호 기반 광고 삽입에서 돌아오는 경우 재생이 잘못된 URL로 중단됩니다.
...
* (ZD 파섹#34385) - 신호 기반 광고 삽입에서 돌아올 때 재생이 잘못된 URL로 정지합니다.
   ...
   매니페스트 읽기가 계속 실행될 수 있도록 에 대한 최대 동시 수를 `CustomAVAssetLoaderOperations`늘립니다.
...
* (ZD 파섹#33904) - TVSDK가 AVFfoundation 알림 `AVAudioSessionMediaServicesWereLostNotification` 및 TVSDK에 등록되어 있지 않습니다 `AVAudioSessionMediaServicesWereResetNotification`.

   `PTMediaServicesWereLostNotification` 이제 미디어 서비스가 재설정되거나 손실될 때 알림을 받으려면 플레이어 앱에 등록할 `PTMediaServicesWereResetNotification` 수 있습니다.
...
* (ZD #32146) - iOS 11 개발 베타 차단 시 HLS Live 컨텐츠에 `PTMediaPlayerStatusError` 대해 수신되지 않음

   Charles(Drop 연결 및 403)를 사용하여 차단할 때 HLS Live 및 VOD 컨텐트에 대해 수신되지 `PTMediaPlayerStatusError` 않음
...
* (ZD#33341) - Xcode 9에서 `DRMInterface.h` 빌드 경고를 트리거합니다.
   ...
   매개 변수 목록에서 &#39;void&#39;라는 단어가 누락된 두 블록 프로토타입을 `DRMInterface.h` 수정했습니다.
...
* (ZD 파섹 #32465) - 플레이어에서 병합된 재생 목록을 처리할 수 없습니다.
   ...
   전화 `finishLoadingWithError`문의:오류) AV 재단이 대체 스트림/트리거 장애 조치를 시도하려면 이 오류가 발생합니다.
...
* (ZD #29176) - 충돌 `PTAdPolicyDeligate` 발생 `satAdBreakAsWatched:position`
...
문제가 해결되었습니다. iOS TVSDK는 SDK를 효과적으로 `exception(AUDNetworkAdInfo::initWithAdId)` 처리하지 않습니다. 빈 광고 ID로 인해 예외가 발생했습니다.
...
* (ZD 파섹 #27830) - IOS에서 자막과 자막의 차이를 프로그래밍 방식으로 결정해야 합니다.
...
* (ZD 파섹# 27460) 미드롤 첫 광고 통화 - 403을 `cdn.auditude.com` 반환하는 POST.
...
이 문제는 PTVideoAnalyticsTrackingMetadata에 assetDuration API를 추가하여 라이브/선형 스트림의 자산 지속 시간을 업데이트하고 실시간 스트림 확인에 대한 논리를 제공하여 해결되었습니다.
...
* (ZD 파섹# 25347) AVURLAsset에서 사용자 지정 HTTP 헤더 설정
...
이 문제는 충돌을 일으키는 동시 시청 문제로부터 추가적인 보호를 제공하여 해결되었습니다.
...
* (ZD #18639) - TV SDK가 긴 레코딩 자산에 과도한 CPU/리소스를 사용하고 있습니다.
...
* (ZD 파섹 #18844) IOS 플레이어의 라이브 컨텐츠에 대한 재생 헤드를 추적합니다.
...
* (Zendesk #18008) - iOS8+에 대한 쿠키 지원을 통해 토큰화된 스트림 지원
...
* (Zendesk #18166) - TVSDK 1.4.15는 dSYM 파일 옵션이 있는 DWARF로 컴파일할 때 수백 개의 경고를 제공합니다.
...
TVSDK 라이브러리가 YUV RGB 색상 변환을 수행하는 네온 코드를 사용할 때 Android M OS에서 TVSDK가 `->` 충돌했습니다. 이 문제는 Non-Neon 버전을 사용하여 이 문제를 일으키는 기능을 업데이트하여 해결되었습니다 `code`.
...
빈 광고 브레이크 추적 URL을 ping할 수 있도록 지원하는 TVSDK는 이제 빈 광고 브레이크에 대한 광고 중단 시작 및 완료 페이지를 확인합니다.
...
* (ZD #2141) TreeHouse 앱에 대한 Analytics 구현에서 패키지를 빌드하는 `AdobeAnalyticsPlugin.a` 라이브러리를 추가했습니다.
...
* DRM을 지원하여 API를 추가하여 HTTPS를 강제 `forceHTTPS` 적용할 `isForcingHTTPS` 수 있습니다.
...
* iOS TVSDK에서 모든 광고는 콘텐츠 매니페스트에 붙여집니다. 광고 동작은 컨텐츠 및 광고 세그먼트의 지속 시간을 기반으로 하여 구현됩니다. 따라서 세그먼트 지속 시간이 정확하지 않을 경우 검색 작업이 광고 분리의 시작 또는 끝 프레임에서 항상 끝나지 않을 수 있습니다. 재생 시간이 프레임에 있더라도 플랫폼 자체에서 검색 시 부과하는 허용치가 있으며 일부 프레임 또는 광고 또는 컨텐츠가 표시될 수 있습니다. 이는 플랫폼과 iOS에서 TVSDK로 광고 삽입 작업을 수행하는 방식에 대한 제한 사항입니다.
...
해결 방법:libstdc++.6.dylib 라이브러리를 다음과 같이 명시적으로 지정합니다.Target->빌드 단계->바이너리를 라이브러리와 연결하고 libstdc++.6.dylib를 추가합니다.
* 포스트롤 광고가 대체 API에 삽입되지 않습니다.
* 광고 중단에서 검색 시 중복 광고 시작 및 광고 중단 알림을 발생시킵니다.
...
* 정확한 시간 광고 확인 기능이 활성화된 경우 자세한 오류 알림을 사용할 수 없습니다.
...
* 이 릴리스에서는 HEVC 지원에 다음과 같은 제한 사항이 있습니다.
   ...
   * 4K 지원은 아직 제공되지 않습니다.
   * CMAF에서 지원되지 않으므로 ID3 태그를 지원하지 않습니다.
   * 확인되지 않은 라이브 HEVC 스트림이 확인되지 않았습니다.
   * HEVC 광고 지원이 확인되지 않았습니다.
* JIT가 활성화되고 허용률이 10초로 설정되면 VMAP->VAST 리디렉션 광고의 경우 첫 번째 미드롤 광고 중단에 대해 VAST 호출이 표시되지 않습니다.
