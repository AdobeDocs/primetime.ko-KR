...
(ZD#37820) - 사용자 정의 헤더 HS-Id, HS-SSAI-TAG에 대한 허용 목록을 추가했습니다.
...
* (ZD #32146) - iOS 11 개발 베타 차단 시 HLS Live 컨텐츠에 대해 수신되지 않음 `PTMediaPlayerStatusError`
   ...
   Charles 사용 차단 시 HLS Live 및 VOD 컨텐트에 대해 수신되지 `PTMediaPlayerStatusError` 않음(드롭 연결 및 403)
* (ZD #29242) - 광고가 활성화된 상태에서 비디오 재생 실패

   광고가 활성화되어 있고 AirPlay가 활성화되어 비디오 재생을 시작하면 비디오 재생이 시작되지 않고 오류가 표시되지 않습니다
* (ZD#33341) - Xcode 9에서 `DRMInterface.h` 빌드 경고를 트리거합니다.

   매개 변수 목록에 &#39;void&#39;라는 단어가 없는 두 블록 프로토타입을 `DRMInterface.h` 수정했습니다.
* (ZD#31979) - iPhone 7/iPhone7+용 iOS 10 이상일 때 컴파일/실행하지 않음

   iOS 7 이전 버전에 대한 IB 문서 컴파일이 더 이상 지원되지 않는 문제가 해결되었습니다.
* (ZD#32920) - 광고 중단 및 광고 중단 이수 없음

   광고 브레이크가 광고 인스턴스를 표시하고 있고 광고 인스턴스가 완료된 후에 흰색 빈 화면이 표시됩니다
* (ZD#32509) - iOS 11 화면 녹화 비활성화 iOS 11에서 화면 기록 비활성화

* (ZD#33179) - iOS 11에서 간헐적인 이벤트 실패

   iOS 11에서 이벤트 오류가 해결되었습니다.
...
* (ZD #31951) - 라이센스 회전 중 TVSDK 오류
   ...
   라이센스 순환 문제를 수정했습니다.
* (ZD #31951) - 광고 중단 내에 흰색 빈 화면이 있고 광고 중단 이수 없습니다.
...
새로운 사용자 정의 헤더가 TVSDK 백서 목록에 추가되었습니다.
