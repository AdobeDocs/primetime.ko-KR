---
title: iOS용 TVSDK 3.10 릴리스 노트
description: iOS용 TVSDK 3.10 릴리스 노트는 TVSDK iOS 3.10의 새로운 기능 또는 변경된 사항, 해결되고 알려진 문제 및 장치 문제를 설명합니다.
...
# iOS용 TVSDK 3.10 릴리스 노트 {#tvsdk-for-ios-release-notes}

iOS용 TVSDK 3.10 릴리스 노트는 TVSDK iOS 3.10의 새로운 기능 또는 변경된 사항, 해결되고 알려진 문제 및 장치 문제를 설명합니다.

## 시스템 및 소프트웨어 요구 사항 {#system-software-requirements}

iOS 3.10을 다운로드하기 전에 하드웨어, 운영 체제 및 응용 프로그램 버전이 다음 요구 사항을 충족하는지 확인하십시오.
...
**티켓#37** - CRS 표준화 규칙이 eq 일치 유형으로 실패합니다.

&quot;eq&quot; 일치 유형이 있는 호스트 이름에 대한 마지막 정규화 규칙 집합과 함께 플레이어가 충돌할 때 발생하는 문제가 해결되었습니다.

**버전 3.1**

**티켓 #36313** - 선형 광고 중단 중 간헐적으로 예측 불가능한 결과

라이브 스트림에서 선형 광고 중단 중 간헐적으로 재생되는 문제가 수정되었습니다.

**버전 3.0.1**

**Ticket36948** - CRS - iOS 12에서 자산 선택 순서가 일치하지 않음

CRS에 대해 선택된 자산이 VAST 또는 VMAP 응답에서 반환되는 최고 품질 변형이 항상 그렇지는 않습니다.

**버전 3.0**

**Ticket35311** - 전화 통화 중단 동안 플레이어 상태가 일시 중지되지 않습니다.

플레이어가 중단되지 않도록 막기 위해 인터럽트 처리기가 추가되었습니다. 중단하면 플레이어 상태가 일시 중지됨이 되고 재생 단추를 클릭하면 재생을 다시 시작합니다.

**Ticket36685** - 라이브 에셋 - 플레이어 시간 진행 및 SCTE 마커 시간과 시간이 일치하지 않습니다.

라이브 포인트 앞에 있는 SCTE 마커에 대해 올바른 시간이 계산됩니다.

**Ticket36492** - `currentTime` 일시 중지된 상태 동안 새 위치를 찾을 `localTime` 때 업데이트되지 않음

플레이어가 일시 중지 상태인 경우 이제 플레이어의 현재 시간을 0으로 설정할 수 있습니다.이전에는 재생 상태에서만 0으로 설정되었던 현재 시간을 초과했습니다.

**버전 1.4.45**

**Ticket36294** - Xcode 10에서 iOS TVSDK가 작동하지 않음
...
**Ticket36493** - iOS `libstdc++` 12에서 지원
...
**Ticket34801** - 일시 중지된 상태 동안 새 위치를 찾을 때 현재 시간 및 localTime이 업데이트되지 않았습니다.
