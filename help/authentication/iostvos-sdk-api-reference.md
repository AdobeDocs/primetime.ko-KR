...
- 기본 Primetime 인증 자격 흐름에 대한 설명은 다음을 참조하십시오 [자격 흐름](https://tve.helpdocsonline.com/entitlement-flow).
- 이 API를 사용하여 Primetime 인증 자격 흐름을 구현하는 방법에 대한 단계별 설명은 [iOS 통합 Cookbook](/help/authentication/iostvos-sdk-cookbook.md).
- 최신 iOS AccessEnabler SDK를 보려면 다음 위치로 이동하십시오. <https://tve.zendesk.com/hc/en-us/articles/204963209-iOS-Native-AccessEnabler-Library>. 

 

**참고:** Adobe은 Primetime 인증만 사용하도록 권장합니다 *공용* API:
...
- [getAuthentication:filter](#getAuthN_filter),[ ](#getAuthN_filter)[getAuthentication:withData:](#getAuthN)[andFilter](#getAuthN_filter) - 전체 인증 워크플로우를 시작합니다.
...
- *옵션*: 글로벌 SDK 옵션이 포함된 NSDictionary입니다. 현재 다음 옵션을 사용할 수 있습니다.
   ...
   - **handleSVC** - 프로그래머가 SFSsafariViewController를 처리할지 여부를 나타내는 부울 값 자세한 내용은 [iOS SDK 3.2 이상에서 SFSsafariViewController 지원](https://tve.helpdocsonline.com/sfsafariviewcontroller-support-on-ios-sdk-3.2) 자세한 내용
...
- *requestorID*: 프로그래머와 연결된 고유 ID입니다. Primetime 인증 서비스에 처음 등록할 때 Adobe에서 할당한 고유 ID를 사이트에 전달합니다.
- *signedRequestorID*: **이 매개 변수는 iOS AccessEnabler 버전 1.2 이상에 있습니다.** 개인 키로 디지털 서명된 요청자 ID의 사본. 자세한 내용은 [기본 클라이언트 등록](https://tve.helpdocsonline.com/registering-native-clients).
- *url*: 선택적 매개 변수; 기본적으로 Adobe 서비스 공급자가 사용됩니다(http://sp.auth.adobe.com/). 이 배열을 사용하면 Adobe에서 제공하는 인증 및 인증 서비스의 끝점을 지정할 수 있습니다(다른 인스턴스는 디버깅 목적으로 사용될 수 있음). 이를 사용하여 여러 Primetime 인증 서비스 공급자 인스턴스를 지정할 수 있습니다. 이때 MVPD 목록은 모든 서비스 공급자의 끝점으로 구성됩니다. 각 MVPD는 가장 빠른 서비스 공급자와 연결됩니다. 즉, 먼저 응답하고 해당 MVPD를 지원하는 공급자입니다.

**참고:** 호출 없이 `serviceProviders` 매개 변수 인 라이브러리는 기본 서비스 공급자에서 구성을 검색합니다(예: `https://sp.auth.adobe.com` 프로덕션 프로파일 또는 `https://sp.auth-staging.adobe.com` 용). 만약 `serviceProviders` 매개 변수가 제공되면 URL의 배열이어야 합니다. 지정한 모든 끝점에서 구성 정보를 검색하고 병합됩니다. 서로 다른 서비스 공급자 응답에 중복 정보가 있는 경우 가장 빠른 응답 서버(즉, 가장 짧은 응답 시간이 있는 서버가 우선함)를 위해 충돌이 해결됩니다.
...
**에 의해 트리거됨: **[setOptions:](#setOptions) 이전 [setSelectedProvider:](#setSelProv) 
...
마지막으로 AccessEnabler가 [`setAuthenticationStatus()`](http://tve.helpdocsonline.com/ios-tvos-api-reference$setAuthNStatus) 상태 코드가 0인 콜백으로 로그아웃 흐름의 성공을 나타냅니다.
...
- *keyDictionary*: 사전 데이터 구조(다음 형식):
   ...
   **참고:** 프로그래머가 사용할 수 있는 실제 사용자 메타데이터는 MVPD가 사용할 수 있는 항목에 따라 다릅니다.  이 목록은 Primetime 인증 시스템에 새로운 메타데이터를 사용할 수 있게 되어 추가됨에 따라 확장됩니다.

**콜백이 트리거됨:** [`setMetadataStatus:encrypted:forKey:andArguments:`](#setMetaStatus)

**추가 정보:** [사용자 메타데이터](https://tve.helpdocsonline.com/user-metadata-v2)
...
**추가 정보:** [iOS/tvOS Single Sign On](https://tve.helpdocsonline.com/ios-tvos-sdk-api-reference#)
...
- (BOOL) enablePlatformServices - true인 경우 MVPD는 다음과 같은 SSO 서비스를 지원합니다 [Apple SSO](https://tve.helpdocsonline.com/ios-tvos-sdk-api-reference#).
...
- [자격 흐름](https://tve.helpdocsonline.com/entitlement-flow)
- [Primetime 인증에서 데이터 추적](https://tve.helpdocsonline.com/tracking-data-in-adobe-pass)
