---
title: Dynamic Client Registration이 있는 Android SDK
description: Dynamic Client Registration이 있는 Android SDK
...
# Dynamic Client Registration이 있는 Android SDK {#android-sdk-with-dynamic-client-registration}
...
Android용 Android AccessEnabler SDK가 세션 쿠키를 사용하지 않고 인증을 사용하도록 수정되었습니다. 점점 더 많은 브라우저가 쿠키에 대한 액세스를 제한하므로 인증을 허용하려면 다른 방법을 사용해야 합니다.

Android의 경우 Chrome 사용자 지정 탭의 사용은 다른 애플리케이션의 쿠키에 대한 액세스를 제한합니다.

>**Android SDK 3.0.0** 다음을 소개합니다.

- 동적 클라이언트 등록은 서명된 요청자 ID 및 세션 쿠키 인증을 기반으로 현재 앱 등록 메커니즘을 대체합니다
- 인증 플로우용 Chrome 사용자 지정 탭

>[!NOTE]
>
>Chrome 사용자 지정 탭 지원이 없는 이전 Android 버전의 경우 이전 AccessEnabler SDK 버전과 유사한 WebView 인증을 사용합니다.


## 동적 클라이언트 등록 {#DCR}

Android SDK v3.0+에서는 정의된 대로 동적 클라이언트 등록 절차를 사용합니다. [동적 클라이언트 등록](/help/authentication/dynamic-client-registration.md).
...
- redirectUrl : 고유 url, TVE 대시보드에 명시적으로 추가된 순서를 반대로 도메인 중 하나 또는 *null* string.xml에 &quot;redirect\_uri&quot;가 설정된 경우

참고 : 잘못된 softwareStatement 또는 redirectUrl로 인해 응용 프로그램이 AccessEnabler를 초기화하거나 Adobe Pass 인증 및 인증을 위해 응용 프로그램을 등록하지 않습니다
</br>
참고 : strings.xml의 redirectUrl 매개 변수 또는 redirect\_uri는 애플리케이션에 대해 TVE 대시보드에 추가된 도메인의 값이어야 합니다(예: TVE 대시보드에 추가된 도메인 &#39;adobe.com&#39;의 경우 redirectUrl은 &#39;com.adobe&#39;여야 합니다.
 
...
a. Adobe Pass( TVE 대시보드 )에서 software\_statement 및 redirect\_uri 가져오기

나. 이러한 값을 Adobe Pass SDK에 전달하는 두 가지 옵션이 있습니다.
...
AccessEnabler.getInstance(appContext,softwareStatement, redirectUrl)를 호출합니다.
...
- 등록 신청: 사용 **software\_statement**, SDK는 **client\_id, client\_secret, client\_id\_issued\_at, redirect\_url, grant\_types**. 이 정보는 애플리케이션의 내부 스토리지에 저장됩니다.

- 얻다 **access\_token** client\_id, client\_secret 및 grant\_type=&quot;client\_credentials&quot; 사용. 이 액세스\_token은 SDK가 Adobe Pass 서버에 대한 각 호출에서 사용됩니다 
...
| HTTP 400(잘못된 요청) | {&quot;error&quot;: &quot;invalid\_client&quot;} | 클라이언트를 알 수 없어서 클라이언트 인증에 실패했습니다. SDK는 인증 서버에 다시 등록해야 합니다. |
| HTTP 400(잘못된 요청) | {&quot;error&quot;: &quot;unauthorized\_client&quot;} | 인증된 클라이언트가 이 권한 부여 형식을 사용할 수 있는 권한이 없습니다. |

- MVPD에 수동 인증이 필요한 경우 Chrome 사용자 지정 탭이 열려 해당 MVPD를 사용하여 수동적이며 완료되면 닫힙니다
...
- mvpd가 기억함 : setSelectedProvider(mvpd_id)로 이동합니다.
- mvpd를 선택하지 않았습니다. displayProviderDialog
- mvpd가 선택함 : setSelectedProvider(mvpd_id)로 이동합니다.
...
- 로그인 취소됨 : MVPD 선택 재설정
- URL 체계는 인증이 완료되면 캡처하도록 &quot;adobepass://redirect_uri&quot;으로 설정됩니다

e. get/checkAuthorization : SDK에는 다음이 포함됩니다 **access_token** 헤더에서 권한 부여: 베어러 **access_token**
...
- SDK가 Chrome 사용자 지정 탭을 열어 mvpd_id 로그아웃 종단점에 도달합니다. 완료되면 Chrome 사용자 지정 탭이 닫힙니다
- URL 체계는 &quot;adobepass://logout&quot;으로 설정되어 로그아웃이 완료되면 해당 순간을 캡처합니다
- 로그아웃하면 sendTrackingData(새 이벤트(EVENT_LOGOUT,USER_NOT_AUTHENTICATED_ERROR) 및 콜백이 트리거됩니다. setAuthenticationStatus(0,&quot;Logout&quot;)
...
| access_denied | 401 | access\_token이 잘못되었습니다. sdk는 새 access_token을 요청해야 합니다. |
