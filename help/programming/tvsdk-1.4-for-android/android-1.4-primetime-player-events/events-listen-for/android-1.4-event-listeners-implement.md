...
seo-description: 이벤트 핸들러를 사용하면 TVSDK가 이벤트에 응답할 수 있습니다.
...
1. 응용 프로그램이 수신 대기해야 하는 이벤트를 결정합니다.
   ...
   * **필수 이벤트**:모든 재생 이벤트에 대한 의견 수렴
      ...
      >[!IMPORTANT]
      >
      >재생 이벤트는 오류를 포함하여 플레이어 상태를 `onStateChanged` 제공합니다. 모든 상태가 플레이어의 다음 단계에 영향을 줄 수 있습니다.
...
1. `MediaPlayer.PlaybackEventListener.onStateChanged` state `MediaPlayer.PlayerState.INITIALIZING`

1. `MediaPlayer.PlaybackEventListener.onStateChanged` state `MediaPlayer.PlayerState.INITIALIZED`

>[!TIP]
>
>기본 스레드에서 미디어 리소스를 로드합니다. 배경 스레드에 미디어 리소스를 로드하는 경우 이 작업 또는 후속 TVSDK 작업 또는 둘 다에 오류가 발생하고(예: `IllegalStateException`) 종료될 수 있습니다.

* 재생을 준비하는 `MediaPlayer.prepareToPlay`경우 이벤트의 순서는 다음과 같습니다.

1. `MediaPlayer.PlaybackEventListener.onStateChanged` state `MediaPlayerStatus.PREPARING`

1. `MediaPlayer.PlaybackEventListener.onTimelineUpdated` 광고가 삽입된 경우입니다.
1. `MediaPlayer.PlaybackEventListener.onStateChanged` state `MediaPlayerStatus.PREPARED`
...
* 광고 중단의 모든 광고에 대해 다음 사항이 전달됩니다.
   ...
   * `AdPlaybackEventListener.onAdProgress` (광고 재생 중에 여러 번)
   * `AdPlaybackEventListener.onAdClick` (각 클릭)
   * `AdPlaybackEventListener.onAdStart`
...
TVSDK는 서비스 품질(QoS) 이벤트를 전달하여 버퍼링 및 검색 이벤트와 같은 QoS 통계 계산에 영향을 줄 수 있는 이벤트에 대해 애플리케이션에 알립니다.
...
모든 DRM 관련 이벤트에 대한 알림을 받으려면 수신 대기하십시오 `onDRMMetadata(DRMMetadataInfo drmMetadataInfo)`. TVSDK는 `DRMManager` 클래스를 통해 추가 DRM 이벤트를 전달합니다.
