---
description: TV와 유사한 경험을 통해 광고 중간에 라이브 스트림으로 참여할 수 있습니다.
title: 부분 광고 나누기 삽입
...
TV와 유사한 경험을 통해 광고 중간에 라이브 스트림으로 참여할 수 있습니다.

부분 광고 나누기 기능을 사용하면 클라이언트가 중간 롤에서 라이브 스트림을 시작하는 경우 해당 중간 롤에서 시작되는 TV와 같은 경험을 모방할 수 있습니다. TV채널로 전환하고 TV광고가 원활하게 진행된다는 것과 비슷하다.
...
1. 이 기능을 사용하려면 동작을 사용자 정의하려면 사용자 지정 광고 정책 선택기를 구현해야 합니다. AdvertisingFactory 클래스의 사용자 지정 구현이 아직 없는 경우에는 새 AdvertisingFactory 구현을 추가하십시오. createAdPolicySelector 메서드를 재정의합니다. 이 메서드는 AdPolicySelector 구현의 새 인스턴스를 반환합니다.

   참조에 대한 샘플 구현은 아래에 나와 있습니다. 다음 샘플 구현은 com.adobe.mediacore 패키지에서 사용할 수 있습니다. 그러나 쉽게 참조할 수 있도록 간소화되었으며 있는 그대로 사용하지 않는 것이 좋습니다.
