...
<table id="table_FFC56AC5B1E04DA99C9309C0223ABA90"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> 시간 지정 메타데이터의 고유 식별자입니다. 이 값은 일반적으로 cue/tag ID 속성에서 추출됩니다. 그렇지 않으면 고유한 임의 값이 제공됩니다. get <span class="codeph"> Id를 </span>사용합니다. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <span class="codeph"> 메타데이터 </span> </td> 
   <td colname="col02"> 메타데이터 </td> 
   <td colname="col2"> 재생 목록/매니페스트 사용자 지정 태그에서 처리/압축을 푼 정보입니다. 메타데이터 <span class="codeph"> 가져오기를 </span>사용합니다. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <span class="codeph"> name </span> </td> 
   <td colname="col02"> 문자열 </td> 
   <td colname="col2"> 시간 지정 메타데이터의 이름입니다. 유형이 TAG인 <span class="codeph"> 경우 </span>이 값은 cue/tag 이름을 나타냅니다. 유형이 ID <span class="codeph"> 3인 </span>경우 null입니다. get <span class="codeph"> Name을 </span>사용합니다. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <span class="codeph"> time </span> </td> 
   <td colname="col02"> long </td> 
   <td colname="col2"> 이 시간 메타데이터가 스트림에 있는 기본 컨텐츠의 시작을 기준으로 시간 위치(밀리초)입니다. get <span class="codeph"> Time을 </span>사용합니다. </td> 
  </tr> 
  <tr> 
...
| `public long getTime();` | 이 메타데이터가 스트림에 삽입된 기본 컨텐츠의 시작을 기준으로 한 시간 위치입니다. |
| `public Metadata getMetadata();` | 스트림에 삽입된 메타데이터입니다. |
| `public Type getType();` | 시간 지정 메타데이터의 유형을 반환합니다. |
| `public long getId();` | cue/tag 속성에서 추출한 ID를 반환합니다. 그렇지 않으면 고유한 임의 값이 제공됩니다. |
| `public String getName();` | 일반적으로 HLS 태그 이름인 큐의 이름을 반환합니다. |
