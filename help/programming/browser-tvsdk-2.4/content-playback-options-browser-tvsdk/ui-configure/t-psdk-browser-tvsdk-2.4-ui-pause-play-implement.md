...
seo-description: 브라우저 TVSDK 비헤이비어를 추가하여 단추를 일시 정지하고 재생할 수 있습니다.
...
1. 이벤트를 `AdobePSDK.MediaPlayerStatusChangeEvent` 수신하여 오류를 확인하거나 다른 적절한 작업을 수행합니다.

   Browser TVSDK는 일시 중지 또는 재생 메서드가 호출될 때 이 이벤트를 트리거하고 `MediaPlayerStatus.PLAYING` 또는 `MediaPlayerStatus.PAUSED`같은 새 상태를 포함하여 이벤트 개체에 대한 정보를 전달합니다.
