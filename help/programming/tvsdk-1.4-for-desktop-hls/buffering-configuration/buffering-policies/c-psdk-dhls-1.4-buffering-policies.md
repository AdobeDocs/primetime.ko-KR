...
TVSDK는 미디어 재생 시작 전 최소 30초 이상의 재생 버퍼 길이와 미디어 재생 시작 전 최소 5초의 초기 버퍼 시간을 정의합니다. 응용 프로그램에서 `play`을 호출한 후 재생이 시작되기 전에 TVSDK가 미디어를 초기 시간까지 버퍼링하여 실제로 재생을 시작할 때 매끄럽게 시작합니다.
