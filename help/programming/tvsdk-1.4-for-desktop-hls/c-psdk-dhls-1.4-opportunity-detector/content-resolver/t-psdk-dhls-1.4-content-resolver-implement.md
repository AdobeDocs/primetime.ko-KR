...
TVSDK가 새로운 기회를 감지하면 등록된 컨텐츠 해결 방법을 통해 해당 기회를 해결할 수 있는 컨텐츠 해결 방법을 찾습니다. `canResolve` True를 반환하는 첫 번째 값이 영업 기회를 해결하기 위해 선택됩니다. 컨텐츠 해결 프로그램이 없으면 해당 기회를 건너뜁니다. 컨텐츠 확인 프로세스는 일반적으로 비동기적이므로 컨텐츠 확인자는 프로세스가 완료되면 TVSDK에 알릴 책임이 있습니다.

* 컨텐츠 확인자는 TVSDK가 실행해야 하는 타임라인 작업(일반적으로 광고 중단 배치)을 `client.place` 지정하기 위해 호출합니다.
* 컨텐츠 확인자는 확인 프로세스가 성공한 `client.notifyCompleted` 경우 또는 프로세스가 실패한 `client.notifyFailed` 경우 호출합니다.
