...
1. 설정 `fallbackOnInvalidCreative` 선형/인라인 광고의 미디어 유형이 HLS에 유효하지 않을 때 VMAP이 폴백되도록 true로 설정합니다.
...
TVSDK에서 유효한 미디어 유형은 다음과 같습니다. `application/x-shockwave-flash` (VPAID) 및 `application/x-mpegURL` (m3u8).
