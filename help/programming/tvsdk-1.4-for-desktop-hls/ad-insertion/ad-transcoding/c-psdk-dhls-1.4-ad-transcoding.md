...
* `URLTransformer` TVSDK에서 요청한 CRS 및 URL을 변형하는 데 필요한 방법을 설명하는 인터페이스입니다. 애플리케이션은 이 인터페이스를 구현하고 필요한 방법에 대한 구현을 제공할 수 있습니다.
...
* `NetworkConfiguration.urlTransformer` 구현을 설정하기 위해  `NetworkConfiguration` 메타데이터 인스턴스에 제공되는 setter  `URLTransformer` 메서드입니다.
