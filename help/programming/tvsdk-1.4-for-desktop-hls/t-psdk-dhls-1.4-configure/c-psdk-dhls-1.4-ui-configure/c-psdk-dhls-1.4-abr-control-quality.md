...
# 비디오 품질을 위한 응용 비트 전송률(ABR){#adaptive-bit-rates-abr-for-video-quality}
...
<table id="table_AF838E082235406AA359BF1C1A77F85F"> 
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> 재생은 가능한 경우 가장 높은 비트 전송률 프로파일로 점차 전환됩니다. ABR 전환을 위한 정책을 설정할 수 있습니다. 이 정책은 TVSDK가 프로필 간에 전환되는 속도를 결정합니다. 기본값은 <span class="codeph"> MODERATE_POLICY </span>입니다. <p>TVSDK가 더 높은 비트 전송률을 전환할 경우 플레이어는 현재 ABR 정책을 기반으로 전환하기 위한 이상적인 비트 전송률 프로필을 선택합니다. 
...
* `ABRControlParameterBuilder` 도우미 클래스를 사용하여 매개 변수의 하위 집합을 설정합니다(숨겨진 `ABRControlParameter`에서 작동).
...
* `ABRControlParametersBuilder` 생성자는 모든 ABR 매개 변수를 기본 `ABRControlParameters` 개체의 기본값으로 설정합니다.
...
1. `ABRControlParametersBuilder` 도우미 클래스를 인스턴스화하고 Media Player에서 매개 변수를 설정합니다.
...
## ABRControlParameters {#section_02161FD0A73F40ED9CAE17F9AF850483}를 사용하여 적응형 비트 전송률 구성

ABR 컨트롤 값은 `ABRControlParameters`으로만 설정할 수 있지만 언제든지 새 값을 만들 수 있습니다.

ABR 매개 변수를 설정하는 기능은 `ABRControlParametersBuilder` 클래스가 존재하기 전에 지원되었지만 이 기능은 여전히 구성 시 ABR 매개 변수를 설정하는 데 효과적입니다. 그러나 구성 후 개별 매개 변수를 변경하려면 `ABRControlParametersBuilder` 클래스를 사용해야 합니다.
