...
플레이어는 플레이어의 상태를 나타내는 이벤트 범위를 수신할 수 있다.

가정 `PTMediaPlayer` 는 클라이언트 플레이어의 속성입니다. `self.player` 다음 예제에서는 `PTMediaPlayer` 인스턴스. 다음 예제에서는 `addObservers` PTMediaPlayer에 표시되는 메서드는 지침을 설정하고 대부분의 알림을 포함합니다.
...
<table frame="all" colsep="1" rowsep="1" id="table_ios_notifications"> 
 <tbody> 
  ...
  <tr rowsep="1"> 
   ...
   <td colname="2"> 광고가 진행되었습니다. 광고가 재생되는 동안 계속 발송됩니다. </td> 
  </tr> 
  <tr rowsep="1"> 
  ...
  <tr rowsep="1"> 
   ...
   <td colname="2"> 미디어 플레이어가 버퍼링 상태에 들어갑니다. </td> 
  </tr> 
  <tr rowsep="1"> 
   <td colname="1"> <span class="codeph"> PTAudioTrackChangeCompleted </span> </td> 
   <td colname="2"> 현재 재생 중인 미디어의 오디오 트랙 변경이 완료되었습니다. </td> 
  </tr> 
  <tr rowsep="1"> 
   <td colname="1"> <span class="codeph"> PTAudioTrackChangeStarted </span> </td> 
   <td colname="2"> 현재 재생 중인 미디어의 오디오 트랙에 대한 변경이 개시된다. </td> 
  </tr> 
  <tr rowsep="1"> 
   <td colname="1"> <span class="codeph"> PTMediaPlayerItemChangedNotification </span> </td> 
   <td colname="2"> 다름 <span class="codeph"> PTMediaPlayerItem </span> / <span class="codeph"> PTMediaPlayer </span> 이(가) 설정되었습니다. </td> 
  </tr> 
  <tr rowsep="1"> 
  ...
  <tr rowsep="1"> 
   ...
   <td colname="2"> 새로운 자막 및 대체 오디오 트랙( <span class="codeph"> PTMediaSelectionOption </span>). </td> 
  </tr> 
  <tr rowsep="1"> 
   <td colname="1"> <span class="codeph"> PTMediaPlayerNewNotificationEntryAddedNotification </span> </td> 
   <td colname="2"> 새 항목 <span class="codeph"> PTNotification </span> 이(가)에 추가되었습니다. <span class="codeph"> PTNotificationHistoryItem </span> / 현재 <span class="codeph"> PTMediaPlayerItem </span>즉, 알림 이벤트가 알림 기록에 추가되는 경우입니다. </td> 
  </tr> 
  <tr rowsep="1"> 
  ...
  <tr rowsep="1"> 
   ...
   <td colname="2"> 플레이어 상태가 변경되었습니다. 가능한 상태 값: 
  ...
  <tr rowsep="1"> 
   ...
   <td colname="2"> TVSDK에서 구독한 태그가 처음 발생했습니다. </td> 
  </tr> 
  <tr rowsep="1"> 
   <td colname="1"> <span class="codeph"> PTTimedMetadataChangedInBackgroundNotification </span> </td> 
   <td colname="2"> <p>구독한 태그는 백그라운드 매니페스트에서 식별되고 새 태그는 <span class="codeph"> PTTimedMetadata </span> 인스턴스는 이 인스턴스에서 준비됩니다. </p> </td> 
  </tr> 
 </tbody> 
</table>

## 알림에 대한 샘플 핸들러 {#section_D729C2403A234DD09596829D26882ADC}

다음 코드 조각은 알림을 사용할 수 있는 몇 가지 방법을 보여 줍니다.

가져오기 `PTAdBreak` 인스턴스 사용 `PTMediaPlayerAdBreakKey`:
...
설정 `subtitlesOptions` 및 `audioOptions`:
...
가져오기 `PTAd` 인스턴스 사용 `PTMediaPlayerAdKey`:
