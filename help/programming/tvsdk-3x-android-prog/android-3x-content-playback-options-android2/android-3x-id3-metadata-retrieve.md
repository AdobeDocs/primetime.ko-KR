...
1. 에 대한 이벤트 리스너를 `MediaPlayer.TimedMetadataEventListener#onTimedMetadata(TimeMetadata timeMetadata)` 구현하고 `MediaPlayer` 객체에 등록합니다.
   ...
   >[!TIP]
   >
   >사용자 지정 광고 큐는 동일한 `onTimedMetadata` 이벤트를 사용하여 새 태그 감지를 나타냅니다. 이렇게 하면 매니페스트 수준에서 사용자 지정 광고 큐가 검색되고 ID3 태그가 스트림에 포함되기 때문에 혼동이 발생하지 않습니다. 자세한 내용은 사용자 지정 [태그를](../../tvsdk-3x-android-prog/android-3x-advertising/ad-insertion/custom-tags-configure/android-3x-custom-tags-configure.md)참조하십시오.
