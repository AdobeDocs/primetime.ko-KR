...
iOS 앱에서 DRM으로 보호된 자산 URL을 참조할 때 쿼리 문자열 매개 변수를 (MBR) 세트 수준 M3U8 URL에 추가해야 `?faxs=1` 합니다. 예:

```
https://your.domain.com/hls/[...]/index.m3u8?faxs=1
```

쿼리 `faxs=1` 문자열 매개 변수는 컨텐츠가 DRM으로 보호되었음을 알리고 iOS TVSDK에서 이에 따라 DRM 암호 해독 워크플로우를 트리거합니다. DRM으로 보호된 HLS 자산 URL에 다른 플랫폼용으로 지정된 `faxs=1` 태그를 추가할 수도 있습니다.iOS에서 필요에 따라 처리되거나 다른 플랫폼에서 플레이어에서 비작동 방식으로 처리됩니다.
