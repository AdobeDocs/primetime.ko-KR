---
description: 기본적으로 TVSDK는 사용자가 광고 브레이크를 검색할 때 광고 중단이 강제로 발생합니다. 이전 중단 완료에서 경과된 시간이 일정 시간 이내인 경우 광고 나누기를 건너뛸 동작을 사용자 정의할 수 있습니다.
seo-description: 기본적으로 TVSDK는 사용자가 광고 브레이크를 검색할 때 광고 중단이 강제로 발생합니다. 이전 중단 완료에서 경과된 시간이 일정 시간 이내인 경우 광고 나누기를 건너뛸 동작을 사용자 정의할 수 있습니다.
...
기본 TVSDK 광고 중단 동작을 재정의하려면 기본 광고 정책 선택기를 확장할 수 있습니다. 다음과 같은 4가지 광고 중단 정책을 사용할 수 있습니다.
...
* 건너뛰기
   ...
   >[!NOTE]
   >
   >광고가 라이브 포인트에 있을 때 SKIP 광고 중단 정책이 라이브 스트림에 대해 예상대로 작동하지 않을 수 있습니다. 예를 들어 프리롤의 경우 SKIP를 사용하면 광고 브레이크의 끝 부분으로 검색이 이루어지므로 라이브 포인트보다 클 수 있습니다. 이 경우 TVSDK가 광고 중도를 찾을 수 있습니다.
...
* 제거
   ...
   >[!NOTE]
   >
   >광고 중단 정책은 `REMOVE` 더 이상 사용되지 않을 예정이다. 대신 `SKIP` 광고 중단 정책을 사용하는 것이 좋습니다 `REMOVE`.
