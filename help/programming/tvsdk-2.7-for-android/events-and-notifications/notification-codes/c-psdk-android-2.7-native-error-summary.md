  ...
  <tr> 
   ...
   <td colname="col3"> 이 응용 프로그램에 대한 DRM 로컬 저장소를 재설정합니다. DRMManager.resetDRM을 호출합니다. <p>라이선스 서버에서 CRL(인증서 해지 목록) 서버에 연결하여 CRL 파일을 새로 고치지 못하거나 클라이언트 컴퓨터에서 라이선스 서버에서 해지된 라이선스/인증을 요청하고 있습니다. </p> <p>서버 로그에서 오류 코드 111은 MachineTokenInvalid입니다. 그러나, 클라이언트 레벨에서, 에러 코드(111)는 에러 코드(3324)로 번역된다. </p> <p>DRM 라이센스 서버 관리자는 고객의 라이센스 서버가 Adobe CRL 파일을 검색할 수 있는지 확인해야 합니다. 고객이 Tomcat을 사용하는 경우 다음을 확인할 수 있습니다<span class="filepath"> tomcat/temp/</span> 4개의 CRL 파일이 있는지 확인할 디렉터리입니다. </p> 
    <ul id="ul_23B7F1A104AF49E79EA87DB8E15E337E"> 
     ...
     <li id="li_58EC4EDA2B5146188A0FF7B33C91E2FD">tomcat/temp/에 파일이 없는 경우 방화벽/라우팅 문제로 인해 이 라이선스 서버가 Adobe CRL 서버에 연결할 수 없는 것으로 가정할 수 있습니다. </li> 
    </ul> <p>자세한 내용은 <a href="https://helpx.adobe.com/content/dam/help/en/primetime/drm/drm_secure_deployment_guidelines.pdf" format="http" scope="external"> 방화벽 규칙</a>. </p> <p>CRL 파일을 사용할 수 없거나 만료된 경우 라이센스 서버에 연결할 수 있는지 확인해야 합니다. 고객 라이선스 서버에서 네트워크 스니퍼를 열고 서버를 다시 시작한 다음 클라이언트가 서버에서 라이선스를 요청하도록 합니다. 네트워크 트래픽을 관찰하여 다음 URL 끝점에 대한 호출이 성공했는지 여부를 확인할 수 있습니다. <p>팁: 각 파일을 수동으로 다운로드할 수 있는지 여부를 확인하기 위해 브라우저에 다음 CRL URL을 입력할 수도 있습니다. </p> 
