...
>[!NOTE]
>
>이러한 필터링 기술은 날짜/시간 차원에 대해 작동하지 않습니다. 날짜/시간 차원을 필터링하는 유일한 방법은 시작 및 종료 쿼리 문자열 매개 변수(아래에 설명)를 필수 값으로 설정하는 것입니다.

다음 쿼리 문자열 매개 변수에는 API에 대해 예약된 의미가 있습니다(따라서 차원 이름으로 사용할 수 없거나 해당 차원에 대해 필터링할 수 없음).

CMU API 예약 쿼리 문자열 매개 변수:
...
## CMU API 상태 코드 {#cmu-api-status-codes}

| 상태 코드 | 이유 구문 | 설명 |
|:-----------|:---------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 200 | 확인 | 응답에는 &quot;롤업&quot; 및 &quot;드릴다운&quot; 링크가 포함됩니다(해당하는 경우). 보고서는 리소스의 속성(중첩된 &quot;report&quot; 요소/속성)으로 렌더링됩니다. |
| 400 | 잘못된 요청 | 응답 본문에는 요청의 문제를 설명하는 텍스트 메시지가 포함됩니다.  400 잘못된 요청 상태에는 클라이언트 오류와 관련된 유용한 정보를 제공하는 응답 본문(일반/텍스트 미디어 유형)의 설명 텍스트가 동반됩니다. 부적합한 날짜 형식 또는 기존 차원에 적용된 필터와 같은 사소한 시나리오 외에도, 시스템은 즉시 반환되거나 집계될 대량의 데이터가 필요한 쿼리에 대한 응답도 거부합니다. |
...
예(클라이언트라는 단일 지표가 있고 의 사전 집계가 있다고 가정) `year/month/day/...`):
