---
title: 단일 테넌트/정책 및 여러 애플리케이션에 대한 CM 구현
description: 단일 테넌트/정책 및 여러 애플리케이션에 대한 CM 구현
...
# 단일 테넌트/정책 및 여러 애플리케이션에 대한 CM 구현 {#imp-cm}



## 사용 사례

프로그래머 P는 iPhone 애플리케이션, iPad 애플리케이션 및 웹 사이트를 가지고 있다. 이러한 앱 간의 동시 스트림 수를 제한하려면 Adobe CM(Concurrent Monitoring)과 통합해야 합니다. 프로그래머는 동시 사용을 제한하는 정책을 만듭니다. 다음 두 가지 예를 살펴보겠습니다.

첫 번째 정책에는 2개 이하의 동시 스트림을 허용하는 규칙이 하나 포함되어 있습니다. 최신 스트림 재생이 허용됩니다.
두 번째 정책에는 두 개의 규칙이 포함되어 있습니다. 2개 이하의 장치에서 3개 이하의 동시 스트림을 사용할 수 있으며 최신 스트림을 재생할 수 있습니다.


## 하나의 규칙이 있는 정책

첫 번째 정책에는 하나의 단일 규칙이 포함됩니다. 2개 이하의 동시 스트림이 허용되며, 3개의 동시 스트림이 재생되는 경우 시작할 첫 번째 스트림이 중지됩니다.

2개의 앱 + 1개의 웹 사이트를 사용하여 스트림을 시작합니다.

1. 사용자가 iPhone 앱에서 하나의 스트림을 시작하고 iPad 앱에서 하나의 스트림을 시작합니다. 정책은 이를 허용합니다.
1. 그런 다음 사용자는 프로그래머의 웹사이트에서 세 번째 스트림을 시작한다.
1. 정책의 규칙(최대 스트림 2개, 최신 승)을 사용하면 최신 스트림을 재생할 수 있습니다 **시작된 첫 번째 스트림은 정책과 호환되지 않는 것으로 표시되어 중지됩니다.**
